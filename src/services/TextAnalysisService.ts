...
// Temporary interface for LanguageTool matches if not in types
...
class TextAnalysisService {
  ...
  // Academic databases for simulation
  ...
  // Academic papers for simulation
  private academicPapers = [
    {
    ...
    onProgress?: (progress: number, eta: number | null) => void
      ...
      try {
          ...
          if (API_CONFIG.USE_API && API_CONFIG.BASE_URL) {
              try {
                  result = await this.analyzeTextWithApi(text, onProgress);
              } catch (e) {
                  console.error("API failed, using local", e);
                  result = await this.analyzeTextLocally(text, onProgress);
              }
          } else {
              result = await this.analyzeTextLocally(text, onProgress);
          }
   ...
   */
  private recommendLanguageModel(
    documentType: string,
    ...
    if (
      ...
      if (
        ["scientific", "research", "experiment"].includes(docType) ||
        ["scientific", "research", "data-driven"].includes(style)
      ) {
        recommendedModel = "scientific";
      } else if (
        ["statistics", "data analysis", "quantitative"].includes(docType) ||
        ["statistical", "analytical"].includes(style)
      ) {
        recommendedModel = "statistical";
      } else if (
        ["legal", "law", "case study", "legal brief"].includes(docType) ||
    ...
    // Business category mapping
      ...
      ["business", "corporate", "report", "proposal", "memo"].includes(docType)
    ) {
      recommendedCategory = "business";
      recommendedModel = "business";

      if (
        ["marketing", "sales", "promotional", "advertisement"].includes(
          docType,
        ) ||
        ...
        ["technical", "specification", "documentation", "manual"].includes(
          docType,
        ) ||
    ...
    // Specialized category mapping
      ...
      if (
        ["journalism", "news", "article", "blog post"].includes(docType) ||
        ...
        ["documentation", "guide", "manual", "tutorial"].includes(docType) ||
    ...
    // General category mapping
      ...
      if (
        ["creative", "story", "fiction", "narrative"].includes(docType) ||
   ...
   */
  private adjustWeightsByDocumentType(
    documentType: string,
    ...
    if (
      ...
      this.settings.customWeights.plagiarism = Math.max(
        0.5,
        Math.min(
          2.0,
      ...
      this.settings.customWeights.technicalAccuracy = Math.max(
        0.5,
        Math.min(
          2.0,
          this.settings.customWeights.technicalAccuracy +
            adjustmentFactor * 1.5,
        ),
      );
      this.settings.customWeights.grammar = Math.max(
        0.5,
        Math.min(
          2.0,
      ...
      if (["scientific", "research paper", "experiment"].includes(docType)) {
        ...
        ["legal", "law", "legal brief", "case study"].includes(docType)
      ) {
        this.settings.customWeights.plagiarism = Math.max(
          0.5,
          Math.min(
    ...
    // Business category
      ...
      this.settings.customWeights.clarity = Math.max(
        0.5,
        Math.min(
          2.0,
      ...
      if (
        ["marketing", "sales", "promotional", "advertisement"].includes(docType)
      ) {
        this.settings.customWeights.engagement = Math.max(
          0.5,
          Math.min(
            2.0,
        ...
        ["technical", "specification", "documentation"].includes(docType)
      ) {
        this.settings.customWeights.technicalAccuracy = Math.max(
          0.5,
          Math.min(
    ...
    // Creative category (part of general)
      ...
      ["creative", "story", "fiction", "narrative", "poem"].includes(docType)
    ) {
      ...
      this.settings.customWeights.readability = Math.max(
        0.5,
        Math.min(
          2.0,
    ...
    // Default/General category
      ...
      Object.keys(this.settings.customWeights).forEach((key) => {
        ...
        if (
          ...
          this.settings.customWeights[typedKey] !== undefined
        ) {
          this.settings.customWeights[typedKey] = Math.max(
            0.5,
            Math.min(
              2.0,
              (this.settings.customWeights[typedKey] || 1.0) +
   ...
   */
  private async analyzeTextWithApi(
    text: string,
    onProgress?: (progress: number, eta: number | null) => void
  ): Promise<AnalysisResult> {
    try {
      if (onProgress) onProgress(10, 5); // Start progress
   ...
   */
  private async analyzeTextLocally(
    text: string,
    onProgress?: (progress: number, eta: number | null) => void
  ): Promise<AnalysisResult> {
    ...
    // Estimate total time based on word count (approx 1s per 100 words for local processing)
    const estimatedTotalSeconds = Math.max(2, Math.ceil(wordsCount / 80));

    try {
      // Step 1: Initial scores (quickest)
      if (onProgress) onProgress(5, estimatedTotalSeconds);
      ...
      // Step 2: Readability Metrics (medium cost)
      if (onProgress) {
        const elapsed = (Date.now() - startTime) / 1000;
        onProgress(20, Math.max(1, estimatedTotalSeconds - elapsed));
      }
      const readabilityMetrics = this.calculateReadabilityMetrics(text);
      await this.yieldControl();

      // Step 3: Detailed Plagiarism (highest cost)
      if (onProgress) {
        const elapsed = (Date.now() - startTime) / 1000;
        onProgress(40, Math.max(1, estimatedTotalSeconds - elapsed));
      }
      const plagiarismInstances = await this.detectPlagiarism(text);
      await this.yieldControl();

      // Step 4: Detailed Grammar (high cost)
      if (onProgress) {
        const elapsed = (Date.now() - startTime) / 1000;
        onProgress(75, Math.max(1, (estimatedTotalSeconds - elapsed) * 0.5));
      }
      const grammarIssues = await this.detectGrammarIssues(text);
      await this.yieldControl();
      ...
      throw new Error(
        "Failed to analyze text locally: " +
  ...
  private calculatePlagiarismScore(text: string): number {
    ...
    // Academic models
    if (
      modelCategory === "academic" ||
      ["academic-general", "scientific", "statistical", "legal"].includes(
        modelType,
      )
    ) {
    ...
    const baseScore =
      matches * matchWeight + repetitionFactor * repetitionWeight;
  ...
  private calculateGrammarScore(text: string): number {
    ...
    if (
      this.settings.languageModel === "academic-general" ||
    ...
    switch (this.settings.languageModel) {
      case "academic-general":
        ...
        commonErrors = [
          ...
          {
            pattern: /\b(isn't|aren't|wasn't|weren't|don't|doesn't|can't)\b/g,
            penalty: 2,
          }, // Contractions
          ...
          {
            pattern:
              /\b(very|really|extremely|quite|actually) (very|really|extremely|quite|actually)\b/gi,
            penalty: 3,
          }, // Double intensifiers
        ];
        break;
      ...
      case "medical":
        ...
        commonErrors = [
          ...
          {
            pattern:
              /\b(a lot|lots of|tons of|many|several|various|different)\b/gi,
            penalty: 2,
          }, // Vague quantifiers
      ...
      case "documentation":
        ...
        commonErrors = [
          ...
          {
            pattern: /\b(click on|click|select|choose|pick|tap)\b/gi,
            penalty: 1,
          }, // Inconsistent instruction verbs
          { pattern: /\b(may|might|could|should|would)\b/gi, penalty: 1.5 }, // Ambiguous instructions
          {
            pattern: /\b(easy|simple|just|simply|easily|quickly)\b/gi,
            penalty: 1,
    ...
    switch (this.settings.languageModel) {
      ...
      case "creative": {
        ...
        const sentenceLengthVariance =
          this.calculateSentenceLengthVariance(text);
  ...
  private calculateReadabilityScore(text: string): number {
    ...
    let readabilityScore =
      206.835 -
      1.015 * metrics.avgSentenceLength -
      84.6 * (metrics.avgWordLength / 3);
    ...
    switch (this.settings.languageModel) {
      case "scientific": {
        // Scientific writing has specialized readability expectations
        // Technical terminology and complex structures are expected

        // Adjust base score for scientific writing to account for inherent complexity
        readabilityScore += 25; // Add a base boost to account for scientific context

        // Scientific writing typically has longer sentences with complex structures
        if (metrics.avgSentenceLength < 20) {
          // No penalty for shorter sentences in scientific writing
        } else if (metrics.avgSentenceLength > 40) {
          // Only penalize extremely long sentences
          readabilityScore -= Math.min(
            10,
            (metrics.avgSentenceLength - 40) * 0.5,
          );
        }

        // Reward appropriate technical vocabulary in scientific writing
        if (metrics.avgWordLength > 5) {
          // Bonus for appropriate technical terminology
          readabilityScore += Math.min(15, (metrics.avgWordLength - 5) * 3);
        }

        // Adjust for citation patterns common in scientific writing
      ...
      case "academic-general": {
        ...
        if (metrics.avgSentenceLength < 15) {
          ...
          readabilityScore -= Math.min(
            10,
            (15 - metrics.avgSentenceLength) * 1.5,
          );
        }
      ...
      case "creative": {
        ...
        if (metrics.avgSentenceLength > 25) {
          ...
          readabilityScore -= Math.min(
            15,
            (metrics.avgSentenceLength - 25) * 1.5,
          );
        }
        break;
      }
      ...
      case "medical":
        ...
        if (metrics.avgSentenceLength > 30) {
          ...
          readabilityScore -= Math.min(
            15,
            (metrics.avgSentenceLength - 30) * 1.2,
          );
        }
        ...
        if (metrics.totalSentences / Math.max(1, metrics.totalParagraphs) > 5) {
          readabilityScore -= Math.min(
            10,
            (metrics.totalSentences / Math.max(1, metrics.totalParagraphs) -
              5) *
              1.5,
          );
        }
      ...
      case "documentation":
        ...
        if (metrics.avgSentenceLength > 20) {
          ...
          readabilityScore -= Math.min(
            20,
            (metrics.avgSentenceLength - 20) * 1.5,
          );
        }
        ...
        if (metrics.totalSentences / Math.max(1, metrics.totalParagraphs) < 3) {
          readabilityScore += Math.min(
            10,
            5 - metrics.totalSentences / Math.max(1, metrics.totalParagraphs),
          );
        }
        break;
  ...
  private calculateSentenceLengthVariance(text: string): number {
    ...
    const variance =
      sentenceLengths.reduce(
        (sum, length) => sum + Math.pow(length - mean, 2),
        0,
      ) / sentenceLengths.length;
  ...
  private calculateReadabilityMetrics(text: string): ReadabilityMetrics {
    ...
    const syllablesPerWord =
      words.length > 0 ? totalSyllables / words.length : 0;
    ...
    const complexWordPercentage =
      words.length > 0 ? (complexWordCount / words.length) * 100 : 0;
    ...
    if (fleschKincaidReadingEase > 80)
      readingSpeed = 250; // Easy text
    ...
    const normalizedFleschKincaid = Math.max(
      0,
      Math.min(100, fleschKincaidReadingEase),
    );
    ...
    const normalizeGradeLevel = (gradeLevel: number) =>
      Math.max(0, Math.min(100, 100 - gradeLevel * 5));
    ...
    if (avgSentenceLength > 25) {
      improvementSuggestions.push(
        "Consider shortening sentences to improve readability.",
      );
    }

    if (complexWordPercentage > 20) {
      improvementSuggestions.push(
        "Reduce the number of complex words (words with 3+ syllables).",
      );
    }
    ...
    return {
      ...
      fleschKincaidGradeLevel: Math.max(
        0,
        Math.round(fleschKincaidGradeLevel * 10) / 10,
      ),
  ...
  private async detectPlagiarism(text: string): Promise<PlagiarismInstance[]> {
    ...
    try {
      ...
      toast({
        ...
        description:
          "API-based detection unavailable. Using simulation instead.",
        variant: "default",
      });
    }

    // Fall back to simulation if APIs fail or are unavailable
    return await this.detectPlagiarismSimulation(text);
  }
  ...
  private async detectPlagiarismWithApis(
    ...
    if (ENV.API.BASE_URL) {
      try {
        ...
        toast({
          title: "API Error",
          description:
            "OpenAI plagiarism detection failed. Using fallback methods.",
          variant: "default",
        });
      }
    } else {
    ...
    if (results.length === 0 && ENV.API.COHERE_API_KEY) {
      try {
        ...
        toast({
          title: "API Error",
          description:
            "Cohere plagiarism detection failed. Trying next fallback.",
          variant: "default",
        });
      }
    }
    ...
    if (results.length === 0 && ENV.API.HUGGINGFACE_API_KEY) {
      try {
        ...
        toast({
          title: "API Error",
          description:
            "HuggingFace plagiarism detection failed. Using simulation.",
          variant: "default",
        });
      }
    }
    ...
    if (
      results.length === 0 &&
      ENV.API.COPYLEAKS_API_KEY &&
      ENV.API.COPYLEAKS_EMAIL
    ) {
      try {
   ...
   */
  private async detectPlagiarismWithOpenAI(
    text: string,
  ): Promise<PlagiarismInstance[]> {
    const results: PlagiarismInstance[] = [];
   ...
   */
  private splitTextIntoSemanticChunks(
    text: string,
   ...
   */
  private async detectPlagiarismWithCohere(
    text: string,
  ): Promise<PlagiarismInstance[]> {
    const results: PlagiarismInstance[] = [];
    let totalTokensUsed = 0;
   ...
   */
  private async detectPlagiarismWithHuggingFace(
    text: string,
  ): Promise<PlagiarismInstance[]> {
    const results: PlagiarismInstance[] = [];
    let totalTokensUsed = 0;
    ...
    try {
      ...
      if (totalTokensUsed > 0) {
        ...
        UsageService.recordUsage(0, totalTokensUsed);
        console.log(
          `HuggingFace API usage: ~${totalTokensUsed} tokens (estimated)`,
        );
      }
    } catch (error) {
  ...
  private async checkEmbeddingSimilarity(
    text: string,
    embedding: number[],
  ): Promise<{ source: string; similarity: number; url: string }[]> {
    ...
    if (patternMatches > 0) {
      ...
      const database =
        this.academicDatabases[Math.floor(Math.random() * this.academicDatabases.length)];
   ...
   */
  private async detectPlagiarismWithCopyleaks(
    text: string,
  ): Promise<PlagiarismInstance[]> {
    const results: PlagiarismInstance[] = [];
    ...
    try {
      ...
      for (let i = 0; i < paragraphs.length; i++) {
        ...
        if (i > 0 && i % 10 === 0) await this.yieldControl();
  ...
  private containsAcademicPatterns(text: string): boolean {
    const hasAcademicTerms =
      /\b(study|research|analysis|methodology|framework|paradigm|theory)\b/i.test(
        text,
      );
    ...
    return (
      (hasAcademicTerms && hasResultTerms) ||
      (hasAcademicTerms && hasStatisticalTerms) ||
      hasCitationPattern
    );
  }
  ...
  private containsWebContentPatterns(text: string): boolean {
    const hasDefinitionPattern =
      /\b(is defined as|refers to|is a|can be described as)\b/i.test(text);
    const hasListPattern =
      /\b(firstly|secondly|thirdly|finally|in addition|moreover|furthermore)\b/i.test(
        text,
      );
    const hasExplanationPattern =
      /\b(for example|for instance|such as|namely|specifically)\b/i.test(text);

    return (
      (hasDefinitionPattern && hasExplanationPattern) ||
      (hasListPattern && hasExplanationPattern)
    );
  }
   ...
   */
  private async detectPlagiarismSimulation(text: string): Promise<PlagiarismInstance[]> {
    const instances: PlagiarismInstance[] = [];

    // Use class-level academic databases
    ...
    const detectWithGPT4 = async (text: string): Promise<PlagiarismInstance[]> => {
      const results: PlagiarismInstance[] = [];
      const sentences = text.split(/[.!?]+/).filter((s) => s.trim().length > 0);
      ...
      for (let i = 0; i < sentences.length; i++) {
        ...
        // Yield control every 20 sentences to keep UI alive
        if (i > 0 && i % 20 === 0) await this.yieldControl();
        ...
        if (similarityScore >= 40) {
          ...
          if (sentence.includes('study') || sentence.includes('research')) {
            ...
            databaseIndex = Math.min(
              Math.floor(sentence.length % this.academicDatabases.length),
              this.academicDatabases.length - 1
            );
          }

          const selectedDatabase = this.academicDatabases[databaseIndex];
    ...
    const detectWithCopyleaks = async (text: string): Promise<PlagiarismInstance[]> => {
      const results: PlagiarismInstance[] = [];

      // Use class-level web sources
      const webSources = this.webSources;

      // Exact phrase matching (n-gram analysis)
      const exactPhrases = [
        {
          phrase: "according to recent research",
          source: "Academic Writing Database",
          url: "https://academic-phrasebank.manchester.ac.uk/",
        },
        {
          phrase: "it can be concluded that",
          source: "Journal of Academic Writing",
          url: "https://publications.coventry.ac.uk/index.php/joaw/",
        },
        {
          phrase: "the results of this study indicate",
          source: "Research Methods in Education",
          url: "https://www.routledge.com/Research-Methods-in-Education/Cohen-Manion-Morrison/p/book/9780367193409",
        },
        {
          phrase: "previous studies have shown that",
          source: "Academic Corpus",
          url: "https://www.sciencedirect.com/search?qs=previous%20studies%20have%20shown",
        },
        {
          phrase: "this paper aims to address the gap",
          source: "Research Methodology Handbook",
          url: "https://uk.sagepub.com/en-gb/eur/the-sage-handbook-of-qualitative-research/book242504",
        },
      ];
      ...
      for (let i = 0; i < paragraphs.length; i++) {
        ...
        const hasDefinitionPattern =
          /\b(is defined as|refers to|is a|can be described as)\b/i.test(
            paragraph,
          );
        const hasListPattern =
          /\b(firstly|secondly|thirdly|finally|in addition|moreover|furthermore)\b/i.test(
        ...
        const hasExplanationPattern =
          /\b(for example|for instance|such as|namely|specifically)\b/i.test(
            paragraph,
          );
        ...
        if (similarityScore >= 35) {
          const randomSource =
            webSources[Math.floor(Math.random() * webSources.length)];
          const matchPercentage = Math.min(
            90,
            55 + similarityScore / 2 + Math.random() * 10,
          );
          ...
          if (paragraphIndex !== -1) {
            ...
            const searchQuery =
              keyPhrases.length > 0
                ? keyPhrases[
                    Math.floor(Math.random() * keyPhrases.length)
                  ].replace(/\s+/g, "+")
    ...
    const detectWithTurnitin = async (text: string): Promise<PlagiarismInstance[]> => {
      const results: PlagiarismInstance[] = [];

      // Academic papers that would be matched in a real implementation
      const academicPapers = this.academicPapers;
      ...
      for (let i = 0; i < sections.length; i++) {
        ...
        if (i > 0 && i % 5 === 0) await this.yieldControl();
        // Skip short sections
        if (section.split(/\s+/).length < 50) continue;

        // Check for patterns typical in academic papers
        const hasMethods =
          /\b(methodology|method|approach|procedure|experiment|study design)\b/i.test(
            section,
          );
        const hasResults =
          /\b(results|findings|data|analysis|significant|statistical)\b/i.test(
            section,
          );
        const hasDiscussion =
          /\b(discussion|implications|conclusion|suggest|recommend|future research)\b/i.test(
            section,
          );
        const hasCitations =
          (section.match(/\([^)]+\d{4}[^)]*\)/g) || []).length >= 2; // At least 2 citations
        ...
        if (similarityScore >= 60) {
          const randomPaper =
            academicPapers[Math.floor(Math.random() * academicPapers.length)];
          const matchPercentage = Math.min(
            98,
            70 + similarityScore / 4 + Math.random() * 5,
          );
    ...
    const gpt4Results = await detectWithGPT4(text);
    const copyleaksResults = await detectWithCopyleaks(text);
    const turnitinResults = await detectWithTurnitin(text);
    ...
    if (instances.length === 0) {
      ...
      if (sentences.length > 0) {
        ...
        if (sentenceIndex !== -1) {
          ...
          const randomDatabase = this.academicDatabases[Math.floor(Math.random() * this.academicDatabases.length)];
    ...
    return instances
      ...
      .filter(
        (instance, index, self) =>
          ...
          self.findIndex(
            (i) =>
              i.startIndex <= instance.endIndex &&
              i.endIndex >= instance.startIndex,
          ),
      );
  }
  ...
  private async detectGrammarIssues(text: string): Promise<GrammarIssue[]> {
    ...
    try {
      ...
      toast({
        ...
        description:
          "API-based checking unavailable. Using simulation instead.",
        variant: "default",
      });
    }
   ...
   */
  private async detectGrammarIssuesWithApis(
    text: string,
  ): Promise<GrammarIssue[]> {
    const results: GrammarIssue[] = [];

    // Check if we have any grammar API keys available
    const hasGrammarApis =
      ENV.API.LANGUAGETOOL_API_KEY || ENV.API.TEXTGEARS_API_KEY;

    if (!hasGrammarApis) {
      console.log(
        "No grammar checking APIs configured. Using simulation instead.",
      );
    ...
    if (ENV.API.LANGUAGETOOL_API_KEY) {
      try {
        ...
        const languageToolResults =
          await this.detectGrammarIssuesWithLanguageTool(text);
        ...
        toast({
          title: "API Error",
          description:
            "LanguageTool grammar checking failed. Trying alternatives.",
          variant: "default",
        });
      }
    }
    ...
    if (results.length === 0 && ENV.API.TEXTGEARS_API_KEY) {
      try {
        ...
        const textGearsResults =
          await this.detectGrammarIssuesWithTextGears(text);
   ...
   */
  private async detectGrammarIssuesWithLanguageTool(
    text: string,
  ): Promise<GrammarIssue[]> {
    const results: GrammarIssue[] = [];
      ...
      if (text.length <= maxChunkSize) {
        ...
        for (const paragraph of paragraphs) {
          ...
          if (
            ...
            const chunkResults = await this.processLanguageToolChunk(
              currentChunk,
              chunkStartOffset,
            );
            results.push(...chunkResults);
   ...
   */
  private async processLanguageToolChunk(
    text: string,
    ...
    try {
      ...
      if (response.data?.matches) {
        for (const match of response.data.matches) {
          const issueText = text.substring(
            match.offset,
  ...
  private determineSeverity(match: GrammarRuleMatch): "high" | "medium" | "low" {
    ...
    if (match.rule?.category?.id) {
      ...
      if (
        ["GRAMMAR", "TYPOS", "PUNCTUATION", "CONFUSION_RULE"].includes(
          categoryId,
        )
      ) {
        severity = "high";
      ...
      // Medium severity issues
        ...
        ["STYLE", "CASING", "REDUNDANCY", "COLLOQUIALISMS"].includes(categoryId)
      ) {
        severity = "medium";
      }
      // Low severity issues
      else if (
        ["TYPOGRAPHY", "MISC", "CREATIVE_WRITING"].includes(categoryId)
      ) {
        severity = "low";
      }
    }
   ...
   */
  private deduplicateAndSortGrammarIssues(
    issues: GrammarIssue[],
  ): GrammarIssue[] {
   ...
   */
  private async detectGrammarIssuesWithTextGears(
    text: string,
  ): Promise<GrammarIssue[]> {
    const results: GrammarIssue[] = [];
    ...
    try {
      // Call TextGears API
      const response = await axios.get("https://api.textgears.com/check", {
        params: {
          text: text,
          language: "en-US",
   ...
   */
  private async detectGrammarIssuesSimulation(text: string): Promise<GrammarIssue[]> {
    const issues: GrammarIssue[] = [];

    // 1. Enhanced rule-based grammar checking
    // This simulates LanguageTool's comprehensive rule system
    const detectRuleBasedIssues = async (text: string): Promise<GrammarIssue[]> => {
      const ruleBasedIssues: GrammarIssue[] = [];

      // ... Comprehensive grammar rules ... (skipping for match)
      // Actually I should find the loop in detectRuleBasedIssues
      ...
      const grammarRules = [
        ...
        // Commonly confused words (enhanced detection)
        {
          ...
          suggestion:
            "Verify correct usage: 'their' (possessive), 'they're' (they are), 'there' (location)",
          severity: "medium" as const,
        },
        {
          ...
          issueType: "Commonly Confused Words",
          suggestion:
            "Verify correct usage: 'your' (possessive) vs 'you're' (you are)",
          severity: "medium" as const,
        },
        {
          ...
          issueType: "Commonly Confused Words",
          suggestion:
            "Verify correct usage: 'its' (possessive) vs 'it's' (it is)",
          severity: "medium" as const,
        },
        {
          ...
          issueType: "Commonly Confused Words",
          suggestion:
            "Verify correct usage: 'affect' (verb, to influence) vs 'effect' (noun, result)",
          severity: "medium" as const,
        },
        {
          ...
          issueType: "Commonly Confused Words",
          suggestion:
            "Verify correct usage: 'then' (time) vs 'than' (comparison)",
          severity: "medium" as const,
        },
        ...
        {
          pattern:
            /\b(however|therefore|moreover|furthermore|nevertheless|consequently),/gi,
          issueType: "Comma Usage",
          suggestion:
            "Place a comma before, not after, these conjunctive adverbs when they join independent clauses",
          severity: "medium" as const,
        },
        {
          pattern:
            /\b(i|we|they|you|he|she) (is|am|are|was|were) ([\w\s]+?), (but|however|though)/gi,
          issueType: "Comma Splice",
          suggestion:
            "Use a semicolon or period instead of a comma between independent clauses",
          severity: "medium" as const,
        },
      ];
    ...
    const detectStatisticalIssues = async (text: string): Promise<GrammarIssue[]> => {
      ...
      const unusualSequences = [
        {
          sequence: "the it",
          suggestion: "Check for missing or incorrect words",
        },
        ...
        { sequence: "he were", suggestion: "Change to 'he was'" },
        {
          ...
          suggestion: "Use either 'better' or 'more good' but not both",
        },
        {
          sequence: "most highest",
          suggestion: "Use either 'highest' or 'most high' but not both",
        },
        ...
        {
          sequence: "amount of people",
          suggestion: "Change to 'number of people'",
        },
      ];
      ...
      for (let i = 0; i < sentences.length; i++) {
        const sentence = sentences[i];
        if (i > 0 && (i % 30 === 0)) await this.yieldControl();
    ...
    const detectContextualIssues = async (text: string): Promise<GrammarIssue[]> => {
      ...
      for (let i = 0; i < sentences.length; i++) {
        ...
        const homophones = [
          ...
          { pair: ["allude", "elude"], context: /\b(allude|elude)\b/i },
          {
            ...
            context: /\b(allusion|illusion)\b/i,
          },
          { pair: ["altar", "alter"], context: /\b(altar|alter)\b/i },
          { pair: ["ascent", "assent"], context: /\b(ascent|assent)\b/i },
          {
            pair: ["complement", "compliment"],
            context: /\b(complement|compliment)\b/i,
          },
          {
            pair: ["principal", "principle"],
            context: /\b(principal|principle)\b/i,
          },
          {
            pair: ["stationary", "stationery"],
            context: /\b(stationary|stationery)\b/i,
          },
        ...
        if (
          words.length >= 2 &&
          words.length <= 4 &&
          !/\b(I|we|you|they|he|she|it|this|that|these|those)\b/i.test(
            sentence,
          ) &&
          ...
          if (sentenceIndex !== -1) {
            contextualIssues.push({
              ...
              suggestion:
                "This may be an incomplete sentence. Consider revising or combining with another sentence.",
              severity: "medium",
            });
          }
        }
        ...
        if (
          ...
          (
            sentence.match(
              /\band\b|\bbut\b|\bor\b|\byet\b|\bso\b|\bfor\b|\bnor\b/gi,
            ) || []
          ).length >= 3
        ) {
          const sentenceIndex = text.indexOf(sentence);
          if (sentenceIndex !== -1) {
            contextualIssues.push({
              ...
              suggestion:
                "This sentence is very long with multiple clauses. Consider breaking it into smaller sentences.",
              severity: "medium",
            });
          }
        }
    ...
    const detectStyleIssues = async (text: string): Promise<GrammarIssue[]> => {
      const styleIssues: GrammarIssue[] = [];

      // Detect informal language in formal contexts
      const informalPatterns = [
        {
          pattern: /\b(gonna|wanna|gotta|kinda|sorta)\b/gi,
          suggestion:
            "Use more formal language (e.g., 'going to' instead of 'gonna')",
        },
        {
          pattern: /\b(yeah|nope|nah)\b/gi,
          suggestion: "Use 'yes' or 'no' in formal writing",
        },
        {
          pattern: /\b(kids|guys|folks)\b/gi,
          suggestion:
            "Consider using more formal terms like 'children', 'people', or 'individuals'",
        },
        {
          pattern: /\b(tons of|loads of|a lot of)\b/gi,
          suggestion:
            "Use more precise quantifiers like 'many', 'numerous', or 'a significant number of'",
        },
        {
          pattern: /\b(awesome|cool|amazing|super)\b/gi,
          suggestion:
            "Consider more formal alternatives like 'excellent', 'impressive', or 'exceptional'",
        },
      ];

      // Check for informal language patterns
      for (let i = 0; i < informalPatterns.length; i++) {
        const { pattern, suggestion } = informalPatterns[i];
        if (i > 0) await this.yieldControl();
        let match;
      ...
      const overusedPatterns = [
        {
          ...
          suggestion:
            "Consider using a stronger, more specific word instead of intensifiers",
        },
        {
          pattern: /\b(nice|good|bad|interesting)\b/gi,
          suggestion: "Use more specific and descriptive adjectives",
        },
        {
          pattern: /\b(in conclusion|to sum up|in summary)\b/gi,
          suggestion: "Vary your transitional phrases",
        },
        {
          pattern: /\b(thing|stuff)\b/gi,
          suggestion: "Use more specific nouns",
        },
      ];
      ...
      for (let i = 0; i < overusedPatterns.length; i++) {
        const { pattern } = overusedPatterns[i];
        if (i > 0) await this.yieldControl();
        let match;
        while ((match = pattern.exec(text)) !== null) {
          ...
          if (wordCounts[word] >= 3) {
            const overusedPattern = overusedPatterns.find((p) =>
              p.pattern.test(word),
            );
    ...
    const ruleBasedIssues = await detectRuleBasedIssues(text);
    const statisticalIssues = await detectStatisticalIssues(text);
    const contextualIssues = await detectContextualIssues(text);
    const styleIssues = await detectStyleIssues(text);
